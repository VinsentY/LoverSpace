            ...
            if (password.length() >= 6) {
                BmobSMS.requestSMSCode(phoneNumber, "", new QueryListener<Integer>() {
                    ...
                    public void done(Integer smsId, BmobException e) {
                        if (e == null) {
                            Snackbar.make(btn_submit,"发送验证码成功，短信ID：" + smsId,Snackbar.LENGTH_LONG).show();
                            VerificationActivity.actionStart(PhoneNumberActivity.this, phoneNumber, password);
                        } else {
                            Snackbar.make(btn_submit,"发送验证码失败：" + e.getErrorCode() + "-" + e.getMessage(),Snackbar.LENGTH_LONG).show();
                        }
                    }
                });
            } else {
                Snackbar.make(btn_submit,"输入密码不合法！",Snackbar.LENGTH_LONG).show();
            }
        } else {
            Snackbar.make(btn_submit,"手机号或密码不能为空！",Snackbar.LENGTH_LONG).show();
        }
